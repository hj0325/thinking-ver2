...
function normalizeStage(value) {
  ...
  const trimmed = value.trim();
  const parsed = StageSchema.safeParse(trimmed);
  const stage = parsed.success ? parsed.data : "research-diverge";
  const isIdeation = stage.startsWith("ideation-");
...
function normalizeChatNodeResult(raw) {
  ...
  const user_nodes = user_nodes_all.slice(0, 4);
  const cross_connections = cross_connections_all.slice(0, 3);
...
- Mode: ${mode === "ideation" ? "Design / Ideation" : "Research"}
...
- When stage is research-diverge: focus on clarifying the problem space and context from multiple angles, especially "Why/Who/What".
- When stage is research-converge: prioritize refining and sharpening the main problem statements into fewer, clearer nodes.
- When stage is ideation-diverge: focus more on "How/What" as solution candidates and possible experiments branching out from the problem.
- When stage is ideation-converge: favor fewer nodes that describe concrete solution directions, decisions, or execution plans.
